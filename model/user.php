...
function suppression ($id, $pseu){
    $check_role = recup_role($pseu);
    global $c;
    if (!($check_role == "admin") ){ //or !($_SESSION['role'] == "admin")
    $sql = "DELETE FROM users WHERE id='".$id."'";
    //var_dump($sql);
    mysqli_query($c,$sql);
        $sql = "DELETE FROM commentaires WHERE id_user='".$id."'";
        mysqli_query($c,$sql);
        //echo "<p>votre compte a été supprimé.</p>";
        session_destroy();
        header("location: index.php?page=confirm_suppr");
        //sleep(10);
        //header('Refresh: 1000; URL=http://localhost/semestre_3/projets1/index.php');
