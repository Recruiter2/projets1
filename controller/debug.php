...
voir sql injection
*/
$c = new PDO('mysql:host=localhost;dbname=projets1;charset=utf8', 'l2', 'L2', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));

function commenter($id_user,$id_media,$titre,$text, $c){
    $c;
    $sql = $c->prepare('INSERT INTO `commentaires` (`titre`, `text`, `reactions`, `id_media`, `id_user`) 
VALUES(?, ?, ?, ?, ?)');
...
if (isset($_POST["action"])) {
    if ($_POST["action"] == "envoi_com"){
        if(isset($_POST["titre"])&&isset($_POST["text"])){
            ...
            commenter($user,$media,$titre,$text, $c);
